import {Verdana110 as calculateWidth} from './calc-text-width';
...
export function badgen({
  ...
  const sbTextWidth = label ? calculateWidth(label) : 0;
  const stTextWidth = calculateWidth(status);
...
function sanitize(string_: string): string {
  return string_
    .replaceAll('&', '&amp;')
    .replaceAll('<', '&lt;')
    .replaceAll('>', '&gt;')
...
function createAccessibleText({label, status}: AccessibleTextProperties): string {
